...
void EnemyController::doMove() {
    ...
    if (manager.isCreatureDisabled()){
        std::cout << "Enemy: " << &this->manager << "disabled -> пропускает ход, тк замедлена с предыдущего хода" << std::endl;
    ...
    while (!queue.empty()) {
        ...
        std::vector<Constants::XYPair> nearEntitiesToAttack = this->field.hasNearEntityOfSomeTypesWithAChance(this->typesToAttack,
                                                                                                   tempCoords.x,
    ...
    if (nearestHostileEntitiesCoordinates.empty()){
        ...
        if (availableCoordinates.empty()){
            std::cout << "Enemy id: " << &this->manager << " некого не нашла, ходить тоже не хочет, короче ход здесь постоит(, завершает ход!" << std::endl;
            return;
        }
        ...
        try {
            ...
            std::cout << "Enemy: " << &this->manager << " попало на замедляющую клетку, завершает ход!" << std::endl;
            return;
        }
    }else{
        EntityCoordsWithNearestPositionToAttackFrom priorityEnemy = chooseByPriority(nearestHostileEntitiesCoordinates, distances);
        ...
        if (canAttack) {
            ...
            std::cout  << "Enemy: " << &this->manager << " аттакует сущность в клетке по координатам " + std::to_string(priorityEnemy.entityCoordinates.x) + " " + std::to_string(priorityEnemy.entityCoordinates.y);
        }
    }
    std::cout << "Enemy: " << &this->manager << " штатно завершае ход";
}

EnemyController::EntityCoordsWithNearestPositionToAttackFrom
EnemyController::chooseByPriority(std::vector<EntityCoordsWithNearestPositionToAttackFrom> &enemyWithNearestPositionToAttack, std::vector<std::vector<int>>& distances) {
...
void EnemyController::moveTo(std::stack<Constants::XYPair> trip) {
    while(!trip.empty()){
        ...
        if (this->field.getFieldCells()[stepTo.x][stepTo.y].getCellType() == CellType::Slowing){
            ...
            throw SlowingCellNotification("enemy on slowing cell!");
        }
    }
}
